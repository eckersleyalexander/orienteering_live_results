<template>
  ...
  >
    ...
    >
      <template v-for="(results, resultsI) of column.classes">
        <tr
          :key="`${resultsI}-header`"
          class="headingRow"
    ...
    <div
      id="marquee"
      v-bind:class="{ 'marqueeHidden': resultsResponse.marquee.show == 0, 'marqueeShow': resultsResponse.marquee.show != 0 }"
    >
      <marquee-text
        :repeat="10"
        :duration="Number(resultsResponse.marquee.duration)"
      >{{ resultsResponse.marquee.text }}</marquee-text>
    </div>
  </div>
</template>
  ...
  computed: {
    ...
    pages() {
      ...
      const fit = height => {
        ...
        const overflowV =
          columnHeight + height + this.pageTopPadding + this.pageBottomPadding >
          windowHeight;
